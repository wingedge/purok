        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div class="mb-6 flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                ...
                <div class="flex flex-col gap-3 sm:flex-row sm:items-center">
                    <a href="{{ route('rentals.export') }}" class="inline-flex items-center justify-center bg-gray-800 hover:bg-gray-900 text-white px-5 py-3 rounded-lg font-bold transition shadow">
                        Export CSV
                    </a>

                    <form method="POST" action="{{ route('rentals.import') }}" enctype="multipart/form-data" class="flex flex-col gap-2 sm:flex-row sm:items-center">
                        @csrf
                        ...
                        @foreach($rentals as $rental)
                            <tr class="hover:bg-gray-50 transition-colors">
                                ...
                                <td class="px-6 py-4 text-right text-sm font-medium space-x-3">
                                    @if($rental->status === 'rented')
                                        <form action="{{ route('rentals.return', $rental) }}" method="POST" class="inline">
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" 
